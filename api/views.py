...
def send_email_scheduled(request):
    ...
    if request.method == 'POST':
        ...
        subject = 'Whips & Gigs Vehicle Reminder'
        message = data.get('message')
        send_date_str = data.get('sendDate')

        if message and send_date_str:
            try:
                send_date = datetime.strptime(send_date_str, '%Y-%m-%dT%H:%M:%S.%fZ')
                send_date = send_date.replace(tzinfo=pytz.utc)
                
                send_email_task.apply_async(args=[recipient_email, subject, message], eta=send_date)
            ...
            return JsonResponse({'error': 'Message or sendDate not provided'}, status=400)
    else:
        return JsonResponse({'error': 'Method not allowed'}, status=405)
